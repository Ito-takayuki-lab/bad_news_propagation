...
def custom_speaker_selection_func(last_speaker, groupchat):
    ...
    if(cycle_counter == 5):
        ...
        try:
            chat_result = agents.agent_I.initiate_chat(
                    agents.tester,
                    message="「" + group1_content + "」以上の情報は黒田くんに関する情報です。黒田くんが" + rumor["name"] + "をしているという情報はありますか？",
    ...
    if(cycle_counter == 10):
        ...
        try:
            chat_result = agents.agent_I.initiate_chat(
                    agents.tester,
                    message="「" + group2_content + "」以上の情報は黒田くんに関する情報です。黒田くんが" + rumor["name"] + "をしているという情報はありますか？",
                    summary_method="last_msg",
                    max_turns=1,
                )
            summary = chat_result.summary
    ...
    if(cycle_counter == 15):
        ...
        try:
            chat_result = agents.agent_I.initiate_chat(
                    agents.tester,
                    message="「" + group3_content + "」以上の情報は黒田くんに関する情報です。黒田くんが" + rumor["name"] + "をしているという情報はありますか？",
                    summary_method="last_msg",
                    max_turns=1,
                )
            summary = chat_result.summary
    ...
    if(cycle_counter == 20):
        ...
        try:
            chat_result = agents.agent_I.initiate_chat(
                    agents.tester,
                    message="「" + group4_content + "」以上の情報は黒田くんに関する情報です。黒田くんが" + rumor["name"] + "をしているという情報はありますか？",
                    summary_method="last_msg",
                    max_turns=1,
                )
            summary = chat_result.summary
...
for data_set in data_sets:
    ...
    if data["type"] == "悪事":
        ...
        for rumor in data["contents"]:
            ...
            result_file = "json/result/result_" + agents.llm_name + "_bad_info.json"
            try:
                # グループチャットを作成する。
            ...
            append_to_json(result_file, new_entry)
            max_spread = 0
    ...
    if data["type"] == "慈善活動":
        ...
        for rumor in data["contents"]:
            agents = Agents(rumor["name"])
            result_file = "json/result/result_" + agents.llm_name + "_good_info.json"
            try:
                # グループチャットを作成する。
                group_chat = GroupChat(
                    speaker_selection_method=custom_speaker_selection_func,
